...
const LifeWheelEditor: React.FC<LifeWheelEditorProps> = ({
  ...
  // **ELEGANTE TRANSLATION HELPER FUNCTION**
  const getTranslatedAreaName = useCallback((area: any) => {
    // 1. Zuerst versuchen direkten Translation Key zu finden
    const directKey = `areas.${area.name}`;
    const directTranslation = t(directKey, { defaultValue: null });
    
    // Falls Translation gefunden wurde, verwenden
    if (directTranslation && directTranslation !== directKey) {
      return directTranslation;
    }
    
    // 2. Fallback: Snake_case zu camelCase konvertieren
    const camelCaseKey = area.name
      .split('_')
    ...
    const camelTranslation = t(`areas.${camelCaseKey}`, { defaultValue: null });
    if (camelTranslation && camelTranslation !== `areas.${camelCaseKey}`) {
      return camelTranslation;
    }
    
    // 3. Fallback für bekannte Mappings
    ...
    const mappedKey = knownMappings[area.name];
    if (mappedKey) {
      const mappedTranslation = t(`areas.${mappedKey}`, { defaultValue: null });
      if (mappedTranslation && mappedTranslation !== `areas.${mappedKey}`) {
        return mappedTranslation;
      }
    }
    
    // 4. Letzter Fallback: Original Name mit besserer Formatierung
    return area.name
      .split('_')
