...
type LifeWheelArea = {
  ...
  translations?: any; // JSONB-Feld für Übersetzungen
};

// Typ für die RPC-Antwort (enthält explizit das translations Feld)
type TranslatedLifeWheelArea = LifeWheelArea & {
  translations: any; // JSONB mit Übersetzungen
};
...
export const useTranslatedLifeWheelAreas = (userId: string) => {
  ...
  const fetchAreas = async () => {
    try {
      ...
      // Verwende direkt die übersetzte Abfrage (RPC-Funktion hat UUID-Probleme)
      const translatedData = await fetchTranslatedQuery();
  ...
  const updateArea = async (
    ...
    try {
      ...
      if (error) {
        console.error(
          "Fehler beim Speichern, State bleibt erhalten:",
          error.message,
        );
  ...
  const addArea = async (
    ...
    try {
      ...
      if (error) {
        ...
        setAreas((currentAreas) =>
          currentAreas.filter((area) => area.id !== tempId),
        );
      ...
      if (data && data.length > 0) {
        ...
        setAreas((currentAreas) =>
          currentAreas.map((area) =>
            area.id === tempId ? { ...area, id: realId } : area,
          ),
        );
        return realId;
      }
  ...
  const deleteArea = async (id: string) => {
    try {
      ...
      if (error) {
        ...
        console.error(
          "Fehler beim Löschen, lade aktuelle Daten:",
          error.message,
        );
        fetchAreas();
        return false;
